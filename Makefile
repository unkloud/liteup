...
OPTFLAGS ?= -O2
THREADSAFE ?= 1
FEATURE_FLAGS ?= -DSQLITE_THREADSAFE=$(THREADSAFE) -DSQLITE_ENABLE_FTS4 -DSQLITE_ENABLE_FTS5 -DSQLITE_ENABLE_JSON1 -DSQLITE_ENABLE_RTREE -DSQLITE_ENABLE_EXPLAIN_COMMENTS -DHAVE_READLINE
CFLAGS ?= $(OPTFLAGS) -fPIC -I$(SRC_DIR) $(FEATURE_FLAGS) -pthread -D_REENTRANT
LDFLAGS ?= -pthread
